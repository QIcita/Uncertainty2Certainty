...
class M3DFEL(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.mlla_block = MLLABlock(dim=512, input_resolution=7)
        self.graph_block = GraphBlock(c_out=512, d_model=512,  seq_len=7, gcn_depth=2)
        self.BiIlstm2 = BiImprovedLSTM(input_dim=512, hidden_dim=512, num_layers=3)

        #self.linattn = LinearAttention(dim=1024, input_resolution=7, num_heads=8)

        # self.mlla_block2 = MLLABlock(dim=1024, input_resolution=7)
        # self.graph_block2 = GraphBlock(c_out=1024, d_model=1024,  seq_len=7, gcn_depth=2)
        # self.BiIlstm2 = BiImprovedLSTM(input_dim=1024, hidden_dim=1024, num_layers=2)       
...
class SSM(nn.Module):
    ...
    def forward(self, x):
        # 通过线性变换模拟状态传递
        state = self.W(x) + self.U(x)
        state = self.activation(state)
        return state

class SSLA(nn.Module):
    def __init__(self, dim, input_resolution, num_heads):
        super().__init__()
        self.linear_attn = LinearAttention(dim=dim, input_resolution=input_resolution, num_heads=num_heads, qkv_bias=True)
        self.ssm = SSM(dim)
        self.alpha = nn.Parameter(torch.tensor(0.5))  # 用于融合的可学习权重
    ...
    def forward(self, x):
        ...
        # Linear Attention 输出
        x_attn = self.linear_attn(x_ssm)

        # 自适应融合
        x_combined = self.alpha * x_attn + (1 - self.alpha) * x_ssm
...
class MLLABlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads=4, mlp_ratio=4., qkv_bias=True, drop=0., drop_path=0.,
        ...
        self.attn = SSLA(dim=dim, input_resolution=input_resolution, num_heads=num_heads)
